...
import { DummyFollowingTweets } from "../../utils/DummyFollowingTweets";
import { Constants } from "../../utils/Constants";
import "./TweetFollowingComponent.css";

function TweetFollowingComponent() {
  ...
  const loadFollowingTweets = async () => {
    ...
    setTweets((prevTweets) => [...prevTweets, ...DummyFollowingTweets]); // Add new tweets to existing ones
    setIsLoading(false);
  };

  const handleTweetClick = (tweet) => {
    setLastScrollPosition(window.scrollY); // Save the current scroll position
    setSelectedTweet(tweet); // Update state for the selected tweet on click
  };
  ...
  return (
    <div className="tweet-following-component">
      {selectedTweet ? (
...
export default TweetFollowingComponent;
